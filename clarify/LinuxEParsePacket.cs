...
internal class LinuxEParsePacket : IParsePacket {
  ...
  private void ErrorProcess(byte[] uart_receivebuffer) {
    if (SDKLog.LogEnable("IsErrorEnabled")) {
      SDKLog.Error("{0}Inner Error Packet:{1},Length={2}", reader.LogHeader,
      Util.ToHexString(uart_receivebuffer), uart_receivebuffer.Length);
    }
  ...
  private void loopProcessData() {
    ...
    while (client.IsAlive()) {
      try {
        ...
        // instance another MemoryStream from pcs
        MemoryStream memoryStream = new MemoryStream();

        // calculate data under pcs
        byte b = (byte)pcs.ReadByte();
        if (b != byte.MaxValue) {
          SDKLog.Error("{0}Outer First Packet Header Error,First Header {2}",
          reader.LogHeader, b);
          if (CustomTraceListener.HasConsole) {
        ...
        // save data on tis MemoryStream
        memoryStream.WriteByte(b);

        // calculate data under pcs
        b = (byte)pcs.ReadByte();
        if (b != byte.MaxValue) {
          SDKLog.Error("{0}Outer Second Packet Header Error,Second Header {1}",
          reader.LogHeader, b);
          if (CustomTraceListener.HasConsole) {
        ...
        // save data on this MemoryStream
        ...
        if (b3 != (Util.Checksum(array) ^ num ^ b2)) {
          ...
          SDKLog.Error("{0}CRC Check Error,Current Data:{1}", reader.LogHeader,
                        Util.ToHexString(array2));
          if (CustomTraceListener.HasConsole) {
  ...
  private void ParsePacket(byte[] packetData) {
    ...
    if (packetType == PacketType.HEART_PACK) {
      ...
      reader.result = Result.Module_Is_Closed;
    ...
    if (packetData[1] == 2) {
      ...
      if (b == 0 && b2 == 0) {
        ...
        reader.result = Result.Module_Is_Busy;
        reader.FireReaderErrorEvent();
      } else if (b == 3 && b2 == 9) {
        reader.result = Result.Reverse_Power_Too_Hign;
        reader.FireReaderErrorEvent();
      } else if (b == 3 && b2 == 54) {
        reader.result = Result.Forward_Power_InSufficient;
        reader.FireReaderErrorEvent();
      } else {
        reader.result = Result.Unknown_Exception;
        reader.FireReaderErrorEvent();
      }
      switch (packetType) {
