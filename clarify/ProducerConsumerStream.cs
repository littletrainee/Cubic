  ...
  public void CopyTo() {
    lock (innerStream) {
      if (writePosition > 204800) {
        ...
        if (num > 0) {
          SDKLog.Debug("Read Position={0},Write Position={1},Available \
                        Length={2},Stream Length={3}", readPosition, 
                        writePosition, num, innerStream.Length);
